...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    if (typeof window !== "undefined") {
      const audio = new Audio("/sounds/handgun.mp3");
      try {
        audio.play()
      } catch (error) {
        // Silently handle audio playback errors (e.g., user hasn't interacted yet)
...
export const ThemeToggleButton = ({
  ...
  // Set start dynamically: bottom-right for dark, top-left for light
  // Fallback to "bottom-right" if theme is undefined (during SSR/initial render)
  const start = propStart ?? (resolvedTheme === "dark" ? "bottom-right" : resolvedTheme === "light" ? "top-left" : "bottom-right");
